    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-1">{definitions.length} total</p>
        </div>
        <Link
        ...
        {definitions.length === 0 ? (
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-800">
              {definitions.map(def => (
                <tr key={def.id} className="hover:bg-gray-800 cursor-pointer">
                  <td className="px-4 py-3">
                    <Link href={`/admin/opportunities/${def.id}`} className="block">
                      <div className="text-white font-medium hover:text-violet-400 transition-colors">{def.name}</div>
